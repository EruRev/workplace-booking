...
public interface BookingRepository extends JpaRepository<BookingEntity, UUID> {
    List<BookingEntity> findByUserIdAndIsConfirmed(UUID userId, boolean isConfirmed);

    List<BookingEntity> findByIsConfirmed(boolean isConfirmed);

    List<BookingEntity> findByIsConfirmedAndCancellationDate(boolean isConfirmed, LocalDateTime cancellationDate);

    @Query("SELECT b FROM BookingEntity b " +
            "WHERE b.userId = :id AND :currentDateTime BETWEEN b.bookingStart AND b.bookingEnd")
    ...
    @Query(value = """
            ...
            OR wp.number = :workplaceNumber OR u.full_name = :username OR r.booking_date::DATE = :bookingDate)""",
            nativeQuery = true)
